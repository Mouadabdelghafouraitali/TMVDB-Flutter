...
import 'package:themoviesdb/UI/ByGenres/TVShowByGenre.dart';
...
class TVShowDetails extends StatefulWidget {
  ...
  TVShowDetails({this.id, this.title, this.overview, this.backdropPath, this.posterPath, this.voteAverage, this.heroId});
...
class _TVShowDetailsState extends State<TVShowDetails> {
  ...
  IconData iconData;
  int test = 0;
  AsyncSnapshot<TVShow> snapshotTV;
  ...
  _getContent() {
    return Column(
      children: <Widget>[
              ...
              Container(
                ...
                children: <Widget>[
                  ...
                  FutureBuilder<TVShow>(
                    future: FutureHelper.getTVShow(widget.id),
                        ...
                        return Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              Text(
                                '• Release date : ${Helper.getExactDateWithoutDay(snapshot.data.firstAirDate)}',
                                style: TextStyle(color: Colors.white, height: 1.5),
                              ),
                              Text(
                                '• Runtime : ${Helper.getExactEpisodeTime(snapshot.data.episodeRunTime)}',
                                style: TextStyle(color: Colors.white, height: 1.5),
                              ),
                              Text(
                                '• Network : ${Helper.getExactNetworks(snapshot.data.networks)}',
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(color: Colors.white, height: 1.5),
                              ),
                              Text(
                                '• Next Episode : ${Helper.getNextEpisode(snapshot.data.nextEpisodeToAir ?? null, 'N/A', false)}',
                                style: TextStyle(color: Colors.white, height: 1.5),
                              ),
                            ],
                          ),
        ...
        _getSubContent(snapshotTV)
      ],
    );
  }

  _getSubContent(AsyncSnapshot<TVShow> snapshot) {
    return Column(children: <Widget>[
      _getGenres(snapshot),
      Divider(thickness: 0.2, color: Color(0xFF01d277)),
      _getOverview(),
      Divider(thickness: 0.2, color: Color(0xFF01d277)),
      _getSeasons(snapshot),
      Divider(thickness: 0.2, color: Color(0xFF01d277)),
      ...
      _getCrew(widget.id),
      Divider(thickness: 0.2, color: Color(0xFF01d277)),
      _getProductionCompanies(snapshot),
      Divider(thickness: 0.2, color: Color(0xFF01d277)),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        ...
        FutureBuilder<Videos>(
          future: FutureHelper.getTVShowVideos(id),
  ...
  _getOverview() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        ...
        Text(widget.overview, style: TextStyle(color: Colors.white, fontWeight: FontWeight.normal, fontSize: 14))
      ]),
    );
  }
  ...
  _getGenres(AsyncSnapshot<TVShow> snapshot) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  _addToFavorite(String key) {}

  _getSeasons(AsyncSnapshot<TVShow> snapshot) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: <Widget>[
          Padding(
              padding: const EdgeInsets.only(bottom: 8.0),
              child:
                  Text('• Seasons', textAlign: TextAlign.left, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 24.0))),
          _getListOfSeasons(snapshot)
        ],
      ),
    );
  }

  _getListOfSeasons(AsyncSnapshot<TVShow> snapshot) {
    if (snapshot != null && snapshot.hasData) {
      if (snapshot.data.seasons != null && snapshot.data.seasons.length > 0) {
        List<Season> seasons = snapshot.data.seasons;
        List<Season> reversedSeasonsList = seasons.reversed.toList();
        return Container(
            height: 150.0,
            child: ListView.builder(
                physics: BouncingScrollPhysics(),
                        ...
                        child: Container(
                          ...
                          child: Row(children: <Widget>[
                            SizedBox(
                                ...
                                child: Stack(
                                  children: <Widget>[
                                    Helper.loadImage(Helper.getExactSeasonPoster(reversedSeasonsList[index].posterPath), 150.0,
                                        MediaQuery.of(context).size.width, context, BoxFit.cover),
                                    Visibility(
                                      visible: _getVisibility(reversedSeasonsList, index),
                                      child: Align(
                                        alignment: Alignment.topLeft,
                                        child: SizedBox(
                                            width: 80,
                            ...
                            Expanded(
                              child: Container(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: <Widget>[
                                    Text(reversedSeasonsList[index].name ?? '${widget.title}\'s Season',
                                        maxLines: 2,
                                        textAlign: TextAlign.left,
                                        overflow: TextOverflow.ellipsis,
                                        style: TextStyle(color: Colors.black, fontSize: 16.0, fontWeight: FontWeight.bold)),
                                    Text(
                                        '${Helper.getExactDateYearOnly(reversedSeasonsList[index].airDate)} | ${_getEpisodes(reversedSeasonsList[index].episodeCount ?? 'N/A')}',
                                        style: TextStyle(color: Colors.black, fontSize: 16.0, fontWeight: FontWeight.bold)),
                                    Expanded(
                                      child: Text(Helper.getExactSeasonOverview(reversedSeasonsList[index].overview),
                                          maxLines: 6,
                                          textAlign: TextAlign.left,
                                          overflow: TextOverflow.ellipsis,
                                          style: TextStyle(color: Colors.black, fontSize: 10.0, fontWeight: FontWeight.normal)),
                                    ),
                                  ],
                                ),
                              ),
                    ...
                    onTap: () {
                      Navigator.push(context, MaterialPageRoute(builder: (context) {
                        return SeasonDetails(
                            name: reversedSeasonsList[index].name ?? '${widget.title}\'s Season',
                            id: widget.id,
                            number: reversedSeasonsList[index].seasonNumber ?? 0,
                            //0 as number of the season to avoid the null expection
                            heroId: '${widget.id}season',
                            overview: Helper.getExactSeasonOverview(reversedSeasonsList[index].overview),
                            posterPath: Helper.getExactSeasonPoster(reversedSeasonsList[index].posterPath),
                            episode_count: reversedSeasonsList[index].episodeCount ?? 'N/A',
                            airDate: Helper.getExactDateYearOnly(reversedSeasonsList[index].airDate));
                      }));
                    },
                  );
                },
  ...
  _getGenresList(AsyncSnapshot<TVShow> snapshot) {
    List<Tag> tagList = [];
    if (snapshot != null && snapshot.hasData && snapshot.data.genres != null) {
            ...
            return Padding(
              ...
              child: OutlineButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.push(context, MaterialPageRoute(builder: (context) {
                      return TVShowByGenre(id: id, name: name);
                    }));
                  },
  ...
  _getVisibility(List<Season> seasons, int index) {
    if (seasons[index] == seasons[0])
      return true;
    else
      return false;
  }
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        ...
        FutureBuilder<Reviews>(
          future: FutureHelper.getTVShowReviews(id),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        ...
        FutureBuilder<Credits>(
          future: FutureHelper.getCredits(id, tv_show),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
  ...
  _getProductionCompanies(AsyncSnapshot<TVShow> snapshot) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        Align(
        ...
        _getListProductionCompanies(snapshot)
      ]),
    );
  }
  ...
  _getSimilarMovies(int id) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.start, children: <Widget>[
      ...
      Container(
          height: 190,
          child: FutureBuilder<SimilarTvShow>(
            future: FutureHelper.getSimilarTVShow(id),
            builder: (BuildContext context, AsyncSnapshot<SimilarTvShow> snapshot) {
              if (snapshot.hasData) {
                return ListView.builder(
                    physics: BouncingScrollPhysics(),
                    scrollDirection: Axis.horizontal,
  ...
  _itemBottomSimilar(AsyncSnapshot<SimilarTvShow> snapshot, int index) {
    return Expanded(
      child: Container(
        color: Colors.white,
        child: SizedBox(
          ...
          child: Align(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.center, mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
              Padding(
                padding: const EdgeInsets.only(left: 2.0, right: 2.0),
                child: Text(
                  Helper.getExactName(snapshot, index),
  ...
  _getListProductionCompanies(AsyncSnapshot<TVShow> snapshot) {
    if (snapshot != null && snapshot.hasData) {
